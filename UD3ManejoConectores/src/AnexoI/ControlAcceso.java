...
public class ControlAcceso {
	...
	private void initialize() {
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(usuario.isEmpty() || cont.isEmpty()) {
					...
					if(abd.validar(usuario, cont)) {
						lblMensaje.setText("Bienvenido/a " + abd.nombreCompleto(usuario));
		...
		lblAbierto.setVisible(false);
		lblAbierto.setIcon(new ImageIcon(ControlAcceso.class.getResource("/imagenes/candado_abierto.png")));
		lblAbierto.setBounds(165, 101, 95, 119);
		frame.getContentPane().add(lblAbierto);
		
		lblCerrado = new JLabel("");
		lblCerrado.setIcon(new ImageIcon(ControlAcceso.class.getResource("/imagenes/candado_cerrado.png")));
		lblCerrado.setVisible(false);
